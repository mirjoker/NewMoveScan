...
if __name__ == "__main__":
    defect = "unnecessary_emit"
    total_count = 0
    defect_summary = {}  # 用于存储结果
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".json"):
                ...
                # 只记录有缺陷的数据
                if defect_count > 0:
                    relative_path = os.path.relpath(file_path, directory)  # 转换为相对路径
                    defect_summary[relative_path] = defect_count
                    total_count += defect_count

    # 将总数添加到 defect_summary 中
    ...
    print(f"Total occurrences of {defect}: {total_count}")
